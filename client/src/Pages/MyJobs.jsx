...
const MyJobs = () => {
    const email = "narendravalluru03@gmail.com"
    ...
    <div className="rounded-t mb-0 px-4 py-3 border-0">
      <div className="flex flex-wrap items-center">
        ...
        <div className="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
          <Link to='/post-job'>
          <button className="bg-indigo-500 text-white active:bg-indigo-600 text-xs font-bold uppercase px-3 py-1 rounded outline-none focus:outline-none mr-1 mb-1 
          ease-linear transition-all duration-150" type="button">Post a new job</button>
          </Link>
        </div>
      </div>
    </div>
          ...
          isLoading ? (<div className='flex items-center justify-center h-20'><p>loading...</p></div>) :(<tbody>
            {
              currentJobs.map((job,index) =>(
                ...
                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                  <i className="fas fa-arrow-up text-emerald-500 mr-4"></i>
                  <button onClick={()=>handleDelete(job._id)} className='bg-red-700 text-white py-2 px-2 rounded-sm'>Delete</button>
                </td>
              </tr>
              ))
            }
